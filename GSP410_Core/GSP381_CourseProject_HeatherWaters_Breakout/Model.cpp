...
bool Model::loadModel(LPDIRECT3DDEVICE9 device, LPCWSTR filename){
	HRESULT hr;
    ...
    for(DWORD i = 0; i < materialCount; i++)
    {
		...
		{
			hr = D3DXCreateTextureFromFile( device, (LPCWSTR)modelMaterials[i].pTextureFilename, &textures[i] );
		} else {
			textures[i] = NULL;
		}
    }
...
void Model::setPosition(D3DXVECTOR3 positionVector){
	position = positionVector;
}

void Model::changePosition(D3DXVECTOR3 changeVector)
{
